...
public class FogRenderer implements AutoCloseable {
    ...
    private Vector4f computeFogColor(Camera p_407993_, float p_409359_, ClientLevel p_409862_, int p_409400_, float p_406105_, boolean p_410286_) {
        FogType fogtype = this.getFogType(p_407993_, p_410286_);
        Entity entity = p_407993_.getEntity();
        ...
        if (fogenvironment == null) {
            ...
            int i = fogenvironment.getBaseColor(p_409862_, p_407993_, p_409400_, p_406105_);
            float f4 = p_409862_.getLevelData().voidDarknessOnsetRange();
            float f = Mth.clamp((f4 + p_409862_.getMinY() - (float)p_407993_.getPosition().y) / f4, 0.0F, 1.0F);
    ...
    public Vector4f setupFog(Camera p_407701_, int p_407051_, boolean p_405994_, DeltaTracker p_406871_, float p_409892_, ClientLevel p_407936_) {
        float f = p_406871_.getGameTimeDeltaPartialTick(false);
        Vector4f vector4f = this.computeFogColor(p_407701_, f, p_407936_, p_407051_, p_409892_, p_405994_);
        float f1 = p_407051_ * 16;
        FogType fogtype = this.getFogType(p_407701_, p_405994_);
        Entity entity = p_407701_.getEntity();
        ...
        for (FogEnvironment fogenvironment : FOG_ENVIRONMENTS) {
            if (fogenvironment.isApplicable(fogtype, entity)) {
                fogenvironment.setupFog(fogdata, entity, p_407701_.getBlockPosition(), p_407936_, f1, p_406871_);
                break;
            }
        }
    ...
    private FogType getFogType(Camera p_409791_, boolean p_410079_) {
        FogType fogtype = p_409791_.getFluidInCamera();
        if (fogtype == FogType.NONE) {
            return p_410079_ ? FogType.DIMENSION_OR_BOSS : FogType.ATMOSPHERIC;
        } else {
            return fogtype;
        }
    }
