...
public class FishingHookRenderer extends EntityRenderer<FishingHook, FishingHookRenderState> {
    ...
    public void submit(FishingHookRenderState p_431626_, PoseStack p_422771_, SubmitNodeCollector p_427476_, CameraRenderState p_424754_) {
        ...
        p_427476_.submitCustomGeometry(p_422771_, RenderType.lines(), (p_423174_, p_426120_) -> {
            int i = 16;

            for (int j = 0; j < 16; j++) {
                float f3 = fraction(j, 16);
                float f4 = fraction(j + 1, 16);
                stringVertex(f, f1, f2, p_426120_, p_423174_, f3, f4);
                stringVertex(f, f1, f2, p_426120_, p_423174_, f4, f3);
            }
        });
        p_422771_.popPose();
