...
public class AvatarRenderer<AvatarlikeEntity extends Avatar & ClientAvatarEntity>
    ...
    public AvatarRenderer(EntityRendererProvider.Context p_426442_, boolean p_428946_) {
        ...
        this.addLayer(
            new HumanoidArmorLayer<>(
                this,
                ArmorModelSet.bake(
                    p_428946_ ? ModelLayers.PLAYER_SLIM_ARMOR : ModelLayers.PLAYER_ARMOR, p_426442_.getModelSet(), p_424272_ -> new PlayerModel(p_424272_, p_428946_)
                ),
    ...
    public ResourceLocation getTextureLocation(AvatarRenderState p_430303_) {
        return p_430303_.skin.body().texturePath();
    }
    ...
    public void renderRightHand(PoseStack p_428282_, SubmitNodeCollector p_424928_, int p_427204_, ResourceLocation p_423115_, boolean p_431039_) {
        if (!net.minecraftforge.client.event.ForgeEventFactoryClient.onRenderArm(p_428282_, p_424928_, p_427204_, HumanoidArm.RIGHT))
        this.renderHand(p_428282_, p_424928_, p_427204_, p_423115_, this.model.rightArm, p_431039_);
    }

    public void renderLeftHand(PoseStack p_424615_, SubmitNodeCollector p_425532_, int p_427068_, ResourceLocation p_425626_, boolean p_423290_) {
        if (!net.minecraftforge.client.event.ForgeEventFactoryClient.onRenderArm(p_424615_, p_425532_, p_427068_, HumanoidArm.LEFT))
        this.renderHand(p_424615_, p_425532_, p_427068_, p_425626_, this.model.leftArm, p_423290_);
    }

    private void renderHand(
        PoseStack p_428166_, SubmitNodeCollector p_424874_, int p_425901_, ResourceLocation p_428901_, ModelPart p_423467_, boolean p_423600_
    ) {
        ...
        p_424874_.submitModelPart(p_423467_, p_428166_, RenderType.entityTranslucent(p_428901_), p_425901_, OverlayTexture.NO_OVERLAY, null);
    }
