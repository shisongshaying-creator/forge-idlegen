...
public class AtlasManager implements PreparableReloadListener, MaterialSet, AutoCloseable {
    ...
    public TextureAtlas getAtlasOrThrow(ResourceLocation p_427576_) {
        AtlasManager.AtlasEntry atlasmanager$atlasentry = this.atlasById.get(p_427576_);
        if (atlasmanager$atlasentry == null) {
            throw new IllegalArgumentException("Invalid atlas id: " + p_427576_);
        } else {
            return atlasmanager$atlasentry.atlas();
        }
    }

    public void forEach(BiConsumer<ResourceLocation, TextureAtlas> p_429856_) {
        this.atlasById.forEach((p_427635_, p_424411_) -> p_429856_.accept(p_427635_, p_424411_.atlas));
    }
    ...
    public TextureAtlasSprite get(Material p_430287_) {
        ...
        if (textureatlassprite != null) {
            ...
            ResourceLocation resourcelocation = p_430287_.atlasLocation();
    ...
    public CompletableFuture<Void> reload(
        ...
        return atlasmanager$pendingstitchresults.allReadyToUpload
            ...
            .thenAcceptAsync(p_430960_ -> this.materialLookup = atlasmanager$pendingstitchresults.joinAndUpload(), p_426574_);
    }

    @OnlyIn(Dist.CLIENT)
    public record AtlasConfig(ResourceLocation textureId, ResourceLocation definitionLocation, boolean createMipmaps, Set<MetadataSectionType<?>> additionalMetadata) {
        public AtlasConfig(ResourceLocation p_428125_, ResourceLocation p_430165_, boolean p_423864_) {
            this(p_428125_, p_430165_, p_423864_, Set.of());
        }
    }
    ...
    record PendingStitch(AtlasManager.AtlasEntry entry, CompletableFuture<SpriteLoader.Preparations> preparations) {
        public void joinAndUpload(Map<Material, TextureAtlasSprite> p_428167_) {
            ...
            spriteloader$preparations.regions()
                .forEach((p_425855_, p_426885_) -> p_428167_.put(new Material(this.entry.config.textureId, p_425855_), p_426885_));
        }
    }
    ...
    public static class PendingStitchResults {
        ...
        PendingStitchResults(
            List<AtlasManager.PendingStitch> p_429294_,
            Map<ResourceLocation, CompletableFuture<SpriteLoader.Preparations>> p_431643_,
            CompletableFuture<?> p_429588_
        ) {
        ...
        public CompletableFuture<SpriteLoader.Preparations> get(ResourceLocation p_424233_) {
            return Objects.requireNonNull(this.stitchFuturesById.get(p_424233_));
        }
    }
}