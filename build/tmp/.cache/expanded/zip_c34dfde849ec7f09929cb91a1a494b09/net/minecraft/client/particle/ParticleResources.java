...
public class ParticleResources implements PreparableReloadListener {
    ...
    public CompletableFuture<Void> reload(
        ...
        CompletableFuture<List<ParticleDefinition>> completablefuture = CompletableFuture.<Map<ResourceLocation, Resource>>supplyAsync(
            ...
            .thenCompose(
                p_426813_ -> {
                    List<CompletableFuture<ParticleDefinition>> list = new ArrayList<>(p_426813_.size());
                    p_426813_.forEach(
                        (p_428838_, p_429167_) -> {
                            ResourceLocation resourcelocation = PARTICLE_LISTER.fileToId(p_428838_);
                            list.add(
                                CompletableFuture.supplyAsync(
                                    () -> new ParticleDefinition(resourcelocation, this.loadParticleDescription(resourcelocation, p_429167_)), p_424245_
                                )
                            );
                        }
                    );
        ...
        return CompletableFuture.allOf(completablefuture, completablefuture1).thenCompose(p_424882_::wait).thenAcceptAsync(p_424900_ -> {
            ...
            completablefuture.join().forEach(p_431003_ -> {
                Optional<List<ResourceLocation>> optional = p_431003_.sprites();
                if (!optional.isEmpty()) {
    ...
    private Optional<List<ResourceLocation>> loadParticleDescription(ResourceLocation p_425053_, Resource p_429362_) {
        if (!this.spriteSets.containsKey(p_425053_)) {
            LOGGER.debug("Redundant texture list for particle: {}", p_425053_);
            ...
            try {
                ...
                throw new IllegalStateException("Failed to load description for particle " + p_425053_, ioexception);
            }
        }
    }
