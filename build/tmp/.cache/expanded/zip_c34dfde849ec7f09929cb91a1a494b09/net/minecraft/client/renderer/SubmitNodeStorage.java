...
public class SubmitNodeStorage implements SubmitNodeCollector {
    ...
    @Override
    public void submitHitbox(PoseStack p_426351_, EntityRenderState p_424154_, HitboxesRenderState p_428288_) {
        this.order(0).submitHitbox(p_426351_, p_424154_, p_428288_);
    }
    ...
    public <S> void submitModel(
        ...
        S p_431574_,
        PoseStack p_429440_,
        RenderType p_423668_,
        int p_424805_,
        int p_425107_,
        int p_423377_,
        @Nullable TextureAtlasSprite p_426883_,
        int p_426628_,
        @Nullable ModelFeatureRenderer.CrumblingOverlay p_431344_
    ) {
        this.order(0).submitModel(p_430836_, p_431574_, p_429440_, p_423668_, p_424805_, p_425107_, p_423377_, p_426883_, p_426628_, p_431344_);
    }
    ...
    public void submitModelPart(
        ...
        boolean p_426000_,
        int p_422790_,
        ModelFeatureRenderer.CrumblingOverlay p_428539_,
        int p_431905_
    ) {
        this.order(0).submitModelPart(p_429685_, p_430296_, p_426739_, p_422930_, p_429845_, p_428170_, p_426549_, p_426000_, p_422790_, p_428539_, p_431905_);
    }
    ...
    public void submitBlockModel(
        ...
        this.order(0).submitBlockModel(p_431019_, p_427552_, p_423693_, p_425572_, p_426359_, p_429142_, p_429693_, p_427830_, p_429931_);
    }
    ...
    public void submitItem(
        ...
        RenderType p_430297_,
        ItemStackRenderState.FoilType p_427510_
    ) {
        this.order(0).submitItem(p_427166_, p_426551_, p_431446_, p_427702_, p_427528_, p_424406_, p_431271_, p_430297_, p_427510_);
    }

    @Override
    public void submitCustomGeometry(PoseStack p_423586_, RenderType p_424600_, SubmitNodeCollector.CustomGeometryRenderer p_423841_) {
        this.order(0).submitCustomGeometry(p_423586_, p_424600_, p_423841_);
    }
