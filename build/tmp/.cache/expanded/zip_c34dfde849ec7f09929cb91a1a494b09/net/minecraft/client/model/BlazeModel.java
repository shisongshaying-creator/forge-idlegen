...
public class BlazeModel extends EntityModel<LivingEntityRenderState> {
    ...
    public void setupAnim(LivingEntityRenderState p_364987_) {
        ...
        for (int i = 0; i < 4; i++) {
            this.upperBodyParts[i].y = -2.0F + Mth.cos((i * 2 + p_364987_.ageInTicks) * 0.25F);
        ...
        for (int j = 4; j < 8; j++) {
            this.upperBodyParts[j].y = 2.0F + Mth.cos((j * 2 + p_364987_.ageInTicks) * 0.25F);
        ...
        for (int k = 8; k < 12; k++) {
            this.upperBodyParts[k].y = 11.0F + Mth.cos((k * 1.5F + p_364987_.ageInTicks) * 0.5F);
