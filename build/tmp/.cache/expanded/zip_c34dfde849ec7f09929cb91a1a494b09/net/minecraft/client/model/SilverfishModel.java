...
public class SilverfishModel extends EntityModel<EntityRenderState> {
    ...
    public void setupAnim(EntityRenderState p_368765_) {
        ...
        for (int i = 0; i < this.bodyParts.length; i++) {
            this.bodyParts[i].yRot = Mth.cos(p_368765_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI)
                * (float) Math.PI
                * 0.05F
                * (1 + Math.abs(i - 2));
            this.bodyParts[i].x = Mth.sin(p_368765_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.2F * Math.abs(i - 2);
        }
