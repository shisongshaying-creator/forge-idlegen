...
public class EndermiteModel extends EntityModel<EntityRenderState> {
    ...
    public void setupAnim(EntityRenderState p_363690_) {
        ...
        for (int i = 0; i < this.bodyParts.length; i++) {
            this.bodyParts[i].yRot = Mth.cos(p_363690_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI)
                * (float) Math.PI
                * 0.01F
                * (1 + Math.abs(i - 2));
            this.bodyParts[i].x = Mth.sin(p_363690_.ageInTicks * 0.9F + i * 0.15F * (float) Math.PI) * (float) Math.PI * 0.1F * Math.abs(i - 2);
        }
    }
}