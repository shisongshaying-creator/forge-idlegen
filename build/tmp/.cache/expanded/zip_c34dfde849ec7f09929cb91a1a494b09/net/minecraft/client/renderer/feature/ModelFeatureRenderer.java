...
public class ModelFeatureRenderer {
    ...
    private <S> void renderModel(
        ...
        if (p_426187_.outlineColor() != 0 && (p_424552_.outline().isPresent() || p_424552_.isOutline())) {
            p_422455_.setColor(p_426187_.outlineColor());
            VertexConsumer vertexconsumer1 = p_422455_.getBuffer(p_424552_);
        ...
        if (p_426187_.crumblingOverlay() != null && p_424552_.affectsCrumbling()) {
    ...
    public static class Storage {
        ...
        public void add(RenderType p_428600_, SubmitNodeStorage.ModelSubmit<?> p_428367_) {
            if (p_428600_.pipeline().getBlendFunction().isEmpty()) {
                this.opaqueModelSubmits.computeIfAbsent(p_428600_, p_427315_ -> new ArrayList<>()).add(p_428367_);
            } else {
                Vector3f vector3f = p_428367_.pose().pose().transformPosition(new Vector3f());
                this.translucentModelSubmits.add(new SubmitNodeStorage.TranslucentModelSubmit<>(p_428367_, p_428600_, vector3f));
            }
        }
        ...
        public void endFrame() {
            this.opaqueModelSubmits.keySet().removeIf(p_429309_ -> !this.usedModelSubmitBuckets.contains(p_429309_));
