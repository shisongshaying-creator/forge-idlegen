...
public class Connection extends SimpleChannelInboundHandler<JsonElement> {
    ...
    public void tick() {
        ...
        this.pendingRequests
            ...
            .removeIf(
                p_430467_ -> {
                    boolean flag = p_430467_.getValue().timedOut(i);
                    if (flag) {
                        p_430467_.getValue()
                            .resultFuture()
                            .completeExceptionally(
                                new ReadTimeoutException(
                                    "RPC method " + p_430467_.getValue().method().key().location() + " timed out waiting for response"
                                )
                            );
                    }
    ...
    private <Params, Result> CompletableFuture<Result> sendRequest(
        ...
        if (p_428466_) {
            ...
            this.channel.writeAndFlush(JsonRPCUtils.createRequest(i, p_424976_.key().location(), list));
            return completablefuture;
        } else {
            this.channel.writeAndFlush(JsonRPCUtils.createRequest(null, p_424976_.key().location(), list));
            return null;
        }
    }
    ...
    @Nullable
    @VisibleForTesting
    JsonObject handleJsonObject(JsonObject p_428341_) {
        try {
    ...
    @Nullable
    private JsonObject handleIncomingRequest(@Nullable JsonElement p_423022_, String p_425180_, @Nullable JsonElement p_423468_) {
    ...
    @Nullable
    public JsonElement dispatchIncomingRequest(String p_426702_, @Nullable JsonElement p_429101_) {
        ResourceLocation resourcelocation = ResourceLocation.tryParse(p_426702_);
    ...
    @Nullable
    private JsonObject handleError(@Nullable JsonElement p_428115_, JsonObject p_424320_) {
