...
public class PiglinAi {
    ...
    private static void initIdleActivity(Brain<Piglin> p_34892_) {
        p_34892_.addActivity(
            ...
            ImmutableList.of(
                ...
                StartAttacking.<Piglin>create((p_421882_, p_421883_) -> p_421883_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    private static void initCelebrateActivity(Brain<Piglin> p_34921_) {
        p_34921_.addActivityAndRemoveMemoryWhenStopped(
            ...
            ImmutableList.of(
                avoidRepellent(),
                SetEntityLookTarget.create(PiglinAi::isPlayerHoldingLovedItem, 14.0F),
                StartAttacking.<Piglin>create((p_421877_, p_421878_) -> p_421878_.isAdult(), PiglinAi::findNearestValidAttackTarget),
    ...
    protected static void wasHurtBy(ServerLevel p_361749_, Piglin p_34838_, LivingEntity p_34839_) {
        if (!(p_34839_ instanceof Piglin)) {
            ...
            getAvoidTarget(p_34838_).ifPresent(p_421881_ -> {
                if (p_421881_.getType() != p_34839_.getType()) {
    ...
    private static BehaviorControl<LivingEntity> babySometimesRideBabyHoglin() {
        ...
        return CopyMemoryWithExpiry.create(
            p_421885_ -> p_421885_.isBaby() && setentitylooktargetsometimes$ticker.tickDownAndCheck(p_421885_.level().random),
    ...
    protected static void broadcastAngerTarget(ServerLevel p_368320_, AbstractPiglin p_366359_, LivingEntity p_364430_) {
        getAdultPiglins(p_366359_).forEach(p_421876_ -> {
            if (p_364430_.getType() != EntityType.HOGLIN || p_421876_.canHunt() && ((Hoglin)p_364430_).canBeHunted()) {
                setAngerTargetIfCloserThanCurrent(p_368320_, p_421876_, p_364430_);
            }
        });
    }
