...
public class Scoreboard {
            ...
            .flatMap(
                p_391148_ -> {
                    ...
                    return p_391148_.getValue()
                        ...
                        .entrySet()
                        .stream()
                        .map(p_391147_ -> new Scoreboard.PackedScore(s, p_391147_.getKey().getName(), p_391147_.getValue()));
                }
            )
            .toList();
    }
    ...
    protected void loadPlayerScore(Scoreboard.PackedScore p_392188_) {
        ...
        if (objective == null) {
            ...
            this.getOrCreatePlayerInfo(p_392188_.owner).setScore(objective, p_392188_.score);
        }
    }
    ...
    public record PackedScore(String owner, String objective, Score score) {
        public static final Codec<Scoreboard.PackedScore> CODEC = RecordCodecBuilder.create(
            p_393816_ -> p_393816_.group(
                ...
                .apply(p_393816_, Scoreboard.PackedScore::new)
        );
    }
}