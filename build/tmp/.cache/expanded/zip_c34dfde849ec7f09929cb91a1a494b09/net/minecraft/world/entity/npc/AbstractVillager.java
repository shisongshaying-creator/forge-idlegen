...
public abstract class AbstractVillager extends AgeableMob implements InventoryCarrier, Npc, Merchant {
    ...
    @Override
    protected void addAdditionalSaveData(ValueOutput p_410539_) {
        super.addAdditionalSaveData(p_410539_);
        if (!this.level().isClientSide()) {
            MerchantOffers merchantoffers = this.getOffers();
            if (!merchantoffers.isEmpty()) {
                p_410539_.store("Offers", MerchantOffers.CODEC, merchantoffers);
            }
        }

        this.writeInventoryToTag(p_410539_);
    }

    @Override
    protected void readAdditionalSaveData(ValueInput p_407301_) {
        super.readAdditionalSaveData(p_407301_);
        this.offers = p_407301_.read("Offers", MerchantOffers.CODEC).orElse(null);
        this.readInventoryFromTag(p_407301_);
    }
