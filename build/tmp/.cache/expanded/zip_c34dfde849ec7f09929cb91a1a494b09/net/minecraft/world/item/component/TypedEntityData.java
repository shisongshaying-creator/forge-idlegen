    ...
    public static <T> Codec<TypedEntityData<T>> codec(final Codec<T> p_429781_) {
        return new Codec<TypedEntityData<T>>() {
            ...
            public <V> DataResult<Pair<TypedEntityData<T>, V>> decode(DynamicOps<V> p_422773_, V p_425689_) {
                DataResult<T> dataresult = p_422773_.get(p_425689_, "id")
                    .flatMap(p_422657_ -> p_429781_.parse(p_422773_, (V)p_422657_).mapError(p_428724_ -> "Failed to parse 'id': " + p_428724_));
                DataResult<Pair<CompoundTag, V>> dataresult1 = CustomData.COMPOUND_TAG_CODEC.decode(p_422773_, p_422773_.remove(p_425689_, "id"));
                return dataresult.apply2stable(
                    (p_427367_, p_429155_) -> new Pair<>(new TypedEntityData<>(p_427367_, (CompoundTag)p_429155_.getFirst()), (V)p_429155_.getSecond()),
                    dataresult1
                );
            }
